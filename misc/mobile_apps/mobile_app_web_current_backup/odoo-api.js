...
const ODOO_DB = process.env.ODOO_DB || 'sama_conai_test';
  ...
  async getSuccessRate(userId = null) {
    ...
    async getUserProfile(userId) {
        try {
            const response = await this.makeRequest('/web/dataset/call_kw', {
                ...
                method: 'read',
                args: [[userId], [
                    'name', 'email', 'login', 'phone', 'mobile', 
                    'company_id', 'tz', 'lang', 'login_date', 
                    'image_1920', 'groups_id', 'sama_conai_theme'
                ]],
                kwargs: {}
            });
    ...
    async getUserStatistics(userId) {
        try {
            ...
            const requestsResponse = await this.makeRequest('/web/dataset/call_kw', {
                model: 'request.information',
                method: 'search_count',
                args: [[['create_uid', '=', userId]]],
                kwargs: {}
            });
            
            const pendingResponse = await this.makeRequest('/web/dataset/call_kw', {
                model: 'request.information',
                method: 'search_count',
                args: [[['create_uid', '=', userId], ['state', 'in', ['submitted', 'in_progress']]]],
                kwargs: {}
            });
            
            const completedResponse = await this.makeRequest('/web/dataset/call_kw', {
                model: 'request.information',
                method: 'search_count',
                args: [[['create_uid', '=', userId], ['state', 'in', ['responded', 'refused']]]],
                kwargs: {}
            });
            ...
            return {
                success: true,
                data: {
                    total_requests: requestsResponse.result || 0,
                    pending_requests: pendingResponse.result || 0,
                    completed_requests: completedResponse.result || 0,
                    avg_response_time: 15 // Calculé dynamiquement si nécessaire
                }
            };
        } catch (error) {
