  ...
  try {
    ...
    if (isOdooConnected && dataSource === 'odoo_real_data') {
      try {
        ...
        const userRequests = await odooAPI.searchRead(
          'request.information',
  ...
  try {
    ...
    if (isOdooConnected && dataSource === 'odoo_real_data') {
      try {
        ...
        const realRequests = await odooAPI.searchRead(
          'request.information',
          domain,
        ...
        const totalCount = await odooAPI.searchCount(
          'request.information',
          domain
        );
...
// Démarrer le proxy Odoo pour iframe
...
proxyProcess.on('error', (error) => {
  console.log('⚠️  Erreur démarrage proxy Odoo:', error.message);
});
...
app.listen(PORT, () => {
  ...
  console.log(`🔗 Proxy Odoo pour iframe: http://localhost:8078`);
